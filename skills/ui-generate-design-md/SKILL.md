...
- `assets/DESIGN.template.md` is the single definition of the output shape. Instantiate it; do not invent a competing schema and do not edit the template during a run.
...
- Do not modify product source or promote accidental implementation patterns into design decisions.
...
Treat frontmatter shape as a hard gate, not a stylistic choice: a value that is a token group must be a mapping whose children are the schema's fields. Use mappings, never sequences, for token groups. Map source values into DESIGN.md schema fields; preserve semantic token names, not source object nesting. The template records the valid and invalid shapes for each category.
...
URL reconstruction does not change the frontmatter schema. Use only the canonical typography fields listed in the template. Never emit URL-specific aliases such as `family`, `size`, `weight`, `mobile-display`, or nested `sans`/`mono` values inside a typography scale. Reconstructed role names may vary; schema field names may not.
...
Instantiate `assets/DESIGN.template.md`. Never write the document from scratch or from memory: the template carries the frontmatter schema, the canonical section order, and the per-section content rules.

1. Read `assets/DESIGN.template.md` from this skill's directory.
2. Copy it as the base of the output `DESIGN.md`.
3. Replace every `<PLACEHOLDER>` with normalized evidence.
4. Uncomment only the frontmatter categories a governing source supports, and delete every prose section it does not support.
5. Delete every HTML comment inherited from the template and every unresolved placeholder.
6. Run the template's closing checklist, then delete it.

Apply while filling it in:

- Put exact normative values in YAML frontmatter whenever a frontmatter category can represent them. Values the installed specification cannot represent — shadows, container widths, motion timings, z-index contracts, alternate-theme values — belong in prose inside their canonical section.
...
Before saving, inspect the parsed frontmatter and reject it yourself if any `typography` child is scalar or if `rounded` contains keys not present in the governing source.
...
`lint` exits `1` when it reports errors, but severity alone does not decide what to repair. `broken-ref` covers both an unresolved `{path.to.token}` and a component sub-token the specification does not recognize; it is reported as a warning and still means the frontmatter carries a second token schema, so always fix it. The same applies to `section-order`, `unknown-key`, `token-like-ignored`, and any YAML parse warning — a parse warning means the whole frontmatter failed to load and every other finding is meaningless until it is fixed. `missing-primary`, `missing-typography`, `contrast-ratio`, and `orphaned-tokens` report evidence, not shape: they fire when the governing source genuinely has no such token or when a real value falls short. Record them in the final report; never invent a token or edit a measured value to silence them.
